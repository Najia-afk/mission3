...
def safe_eval(x):
    try:
        ...
        print(f"Error evaluating string: {x}, error: {e}")
        return None
...
def group_combinations_with_fuzzy(log_file, threshold=85, generic_name=""):
    ...
    print(f"[{generic_name}] Total combinations to process: {len(combination_log)}")
    ...
    for i, (combination, row) in enumerate(combination_log.iterrows()):
        ...
        if len(combination_key) < 2:
            print(f"[{generic_name}] Skipping combination with insufficient elements: {combination_key}")
            continue
    ...
    print(f"[{generic_name}] Finished processing all combinations.")
...
def fuzzy_dataframe(temp_dir='temp', config_dir='config', checks=None, threshold=85):
    ...
    for fields, generic_name in checks:
        ...
        if not os.path.exists(log_file):
            print(f"[{generic_name}] Log file {log_file} does not exist. Skipping.")
            continue
        ...
        print(f"Grouped results for {generic_name} have been generated")

    print("Fuzzy matching and grouping completed for all checks.")
